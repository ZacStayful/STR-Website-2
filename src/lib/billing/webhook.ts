/**
 * Stripe webhook logic: matching an event back to a profile, and writing the
 * subscription state onto it.
 *
 * Lives here rather than in the route so it can be driven by fake clients in
 * a test. The route is a thin shell that verifies the signature, builds the
 * real dependencies and calls handleStripeEvent().
 *
 * The hard problem this file solves is MATCHING. A subscription set up by hand
 * in the Stripe dashboard never produces a Checkout Session, so there is no
 * client_reference_id to match on — and the profile's stripe_subscription_id is
 * still null, so subscription.* events have nothing to join against either.
 * Those customers used to pay and stay on `plan: 'free'` forever, which is why
 ...
 * Stripe ids it learned onto the profile, and shouts loudly in the logs when a
 * payment cannot be matched to anybody.
 */
...
/**
 * The CRM mirror, injected rather than imported, for two reasons: a test must
 * be able to assert that a pause does NOT log a cancellation date, and the
 * Monday client is a server module the test runner cannot load.
 */
export interface Crm {
  subscriptionStarted(email: string): Promise<void>;
  subscriptionCancelled(email: string): Promise<void>;
}
...
async function mirrorToMonday(
  ...
  if (isLive) {
    await crm.subscriptionStarted(email);
  } else {
    await crm.subscriptionCancelled(email);
  }
}
...
async function syncSubscription(
  ...
  await mirrorToMonday(crm, profile.email ?? email, before, isLive);
}


/**
 * Route one verified Stripe event to the profile it belongs to.
 *
 * Throws on a write failure so the caller can answer 500 and let Stripe retry;
 * an unmatched payment is logged loudly and swallowed, because retrying will
 * not make the profile appear.
 */
    ...
    switch (event.type) {
      ...
      case "checkout.session.completed": {
        ...
        const email =
          ...
          (await customerEmail(deps.stripe, customerId));
        ...
        if (subscriptionId) {
          ...
          await syncSubscription(deps, sub, { userId, email });
          break;
        }

        const profile = await findProfile(deps.admin, { userId, customerId, email });
        if (!profile) {
          console.error(
        ...
        const before = wasLive(profile);
        await applySubscriptionState(deps.admin, profile, {
          status: "active",
          customerId,
          subscriptionId,
          startedAt: new Date().toISOString(),
        });
        await mirrorToMonday(deps.crm, profile.email ?? email, before, true);
        break;
      }
