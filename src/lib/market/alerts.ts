...
export function digestEmail(changes: AlertChange[], siteUrl: string): { subject: string; text: string; html: string } {
  const subject = changes.length === 1 ? `Market Explorer: ${changes[0].name} has changed` : `Market Explorer: ${changes.length} of your saved areas have changed`;
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,'Segoe UI',sans-serif;font-size:15px;line-height:1.55;color:#2e3d2b;max-width:560px">
      ...
      <h1 style="font-size:22px;margin:0 0 14px">Your saved areas moved this week</h1>
      <ul style="padding-left:18px">${changes.map((c) => `<li style="margin:8px 0"><a href="${siteUrl}/markets/${esc(c.code.toLowerCase())}" style="color:#2e3d2b">${esc(line(c))}</a></li>`).join('')}</ul>
      ...
      <p style="color:#7a8274;font-size:12px">You get this because you saved these areas. Turn alerts off under “Edit goals” in the explorer.</p>
    </div>`.trim();
