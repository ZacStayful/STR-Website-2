...
export async function runCollector(opts: { dry: boolean; onlyUserIds?: string[] }): Promise<RunResult> {
  ...
  const tracking = await trackingFor(admin, members);
  ...
  for (const some of chunks(ids)) {
    for (let from = 0; ; from += PAGE) {
      ...
      if (error) return { status: 503, body: { error: `deal_alerts read failed (schema behind?): ${error.message}` } };
  ...
  const keeperPayers = await payersFor([...new Set(keepers.map((k) => k.user_id))]);
  ...
  const memberPayers = await payersFor(ids);
  ...
  if (!opts.dry) {
    for (const some of chunks(inserts, 500)) {
      ...
      if (error) {
        console.error('[collect] deal_alerts write failed:', error.message);
        return { status: 500, body: { error: 'deal_alerts write failed', written } };
      }
      written += data?.length ?? 0;
    }
  }
  const body = { dry: opts.dry, members: members.length, tracked: [...tracking.values()].reduce((n, t) => n + t.load.view.length, 0), candidates: inserts.length, written, ms: Date.now() - started, perUser: perUser.filter((p) => p.alerts.length > 0) };
  console.log('[collect] run', JSON.stringify({ dry: body.dry, members: body.members, tracked: body.tracked, candidates: body.candidates, written, ms: body.ms }));
