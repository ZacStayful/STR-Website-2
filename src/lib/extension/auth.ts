...
export interface ExtensionAccess {
  ...
  // Free reports left, or null for anyone not on the free trial (a subscriber
...
export async function extensionAccess(request: Request): Promise<ExtensionAccess> {
  ...
  const { data: profile } = await createAdminClient().from('profiles').select(`email, ${ACCESS_COLUMNS}, market_goals`).eq('id', token.userId).single();
  if (!profile) return { ...NONE, user: { id: token.userId, email: null }, tokenId: token.id, state: 'blocked' };
  const p = profile as { email: string | null; plan: 'free' | 'pro'; plan_source: string | null; reports_run: number; stripe_subscription_id: string | null; stripe_subscription_status: string | null; market_goals: unknown };
  return {
