...
import { countLargeApplications, planningWindows } from "@/lib/apis/planit";
...
// with no signal, then the stalest, go first; a run stops at its time
// budget and the next run carries on, so the table fills over a few days
// and each area refreshes about monthly (PLANNING_MAX_AGE_DAYS).
//
...
export async function GET(request: Request) {
  ...
  const windows = planningWindows(now);

  const done: { area: string; large_apps_12m: number; large_apps_prev_12m: number }[] = [];
  const failed: string[] = [];
  for (const code of due.slice(0, MAX_PER_RUN)) {
    ...
    const recent = await countLargeApplications(c.lat, c.lng, PLANNING_RADIUS_KM, windows.recent);
    if (recent === null) {
      failed.push(code);
      continue;
    }
    const prior = await countLargeApplications(c.lat, c.lng, PLANNING_RADIUS_KM, windows.prior);
    if (prior === null) {
      failed.push(code);
      continue;
    }
    const { error } = await admin
      ...
      .upsert({ postcode_area: code, lat: c.lat, lng: c.lng, radius_km: PLANNING_RADIUS_KM, large_apps_12m: recent, large_apps_prev_12m: prior, fetched_at: now.toISOString(), source: "planit" }, { onConflict: "postcode_area" });
    if (error) {
      console.error("[planning-signals] upsert failed:", error.message);
      failed.push(code);
      continue;
    }
    done.push({ area: code, large_apps_12m: recent, large_apps_prev_12m: prior });
  }
