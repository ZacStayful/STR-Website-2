import { MockCardGrid, MockFilterBar, MockLevelRow, MockMapPane } from "./ProductMocks";

/** Browser-framed mock of the explorer's find screen, used in the product page hero. */
export function ProductHeroVisual() {
  return (
    <div className="demo-frame mxp-frame">
      <div className="demo-chrome">
        <div className="demo-dots"><span /><span /><span /></div>
        <div className="demo-url">intelligence.stayful.co.uk/markets</div>
        <div className="demo-actions" />
      </div>
      <div className="demo-body mxp-frame-body">
        <MockFilterBar />
        <div className="mxp-frame-grid">
          <div className="mxp-frame-cards">
            <MockLevelRow />
            <MockCardGrid rows={4} dense />
          </div>
          <MockMapPane />
        </div>
      </div>
    </div>
  );
}
