...
export function RateComparison({ estimate, presets, className }: { estimate: EstimateResponse | null; presets: number[]; className?: string }) {
  const plan = estimate?.planCreditPence ?? 730;
  ...
  return (
    <p className={className ?? "text-xs text-muted-foreground"}>
      A full report costs about {formatGbp(plan)} of plan credit or {formatGbp(topup)} of top-up credit. {formatGbp(mid).replace(".00", "")} top-up ≈ {reports(mid, topup)} reports · Starter £19/month ≈ {reports(1900, plan)} reports · Pro £39.99/month ≈ {reports(5000, plan)} reports.
    </p>
  );
}
